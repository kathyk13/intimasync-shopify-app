...
interface ProductRow {
  ...
  ecn: { sku: string; cost: number; qty: number } | null;
}
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const rows: ProductRow[] = await Promise.all(
    matches.map(async (match) => {
      const [eldoradoProduct, honeysplaceProduct, nalpacProduct, ecnProduct] =
        await Promise.all([
          ...
          match.nalpacSku && enabledSuppliers.includes("nalpac")
            ...
            : null,
          match.ecnSku && enabledSuppliers.includes("ecn")
            ? prisma.supplierProduct.findFirst({
                where: { shopId, supplier: "ecn", supplierSku: match.ecnSku },
              })
            : null,
        ]);

      const anyProduct = eldoradoProduct || honeysplaceProduct || nalpacProduct || ecnProduct;
      ...
      return {
        ...
        nalpac: nalpacProduct
          ...
          : null,
        ecn: ecnProduct
          ? { sku: ecnProduct.supplierSku, cost: ecnProduct.cost ?? null, qty: ecnProduct.inventoryQty }
          : null,
      };
    })
  );
...
export default function ProductsIndexPage() {
  ...
  const cheapestSupplier = (row: ProductRow): string | null => {
    ...
    const options = [
      ...
      row.ecn?.qty && row.ecn.qty > 0 ? { s: "ecn", c: row.ecn.cost } : null,
  ...
  const headings = [
    ...
    ...(enabledSuppliers.includes("ecn") ? [{ title: "ECN $" }, { title: "ECN Qty" }] : []),
  ];
  ...
  const rowMarkup = rows.map((row, index) => {
    ...
    return (
      ...
      >
        ...
        {enabledSuppliers.includes("ecn") && (
          <IndexTable.Cell>
            {formatCost(
              row.ecn?.cost,
              row.ecn?.qty,
              cheapest === "ecn",
              row.lockedSupplier === "ecn"
            )}
          </IndexTable.Cell>
        )}
        {enabledSuppliers.includes("ecn") && (
          <IndexTable.Cell>
            <Text as="span" tone={row.ecn?.qty === 0 ? "subdued" : undefined}>
              {row.ecn?.qty ?? "—"}
            </Text>
          </IndexTable.Cell>
        )}
